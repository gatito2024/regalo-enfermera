...
const deliveryRows = [
  ...
  { className: "subtitle-row", words: ["MI", "ENFERMERA", "FAVORITA", "💜"] }
];
...
function resize() {
  ...
  const compactMobile = isMobile && (width < 390 || height < 720);
  const mobileScale = Math.min(width / 390, height / 760);

  document.documentElement.classList.toggle("is-mobile", isMobile);
  document.documentElement.classList.toggle("is-portrait", isPortrait);
  document.documentElement.classList.toggle("is-compact-mobile", compactMobile);
  ...
  const poemTop = isMobile ? height * 0.52 : height * 0.16;
  const nurseBottom = isMobile ? height * 0.12 : height * 0.14;

  // En movil baja las frases al centro visual, encima del sol.
  document.documentElement.style.setProperty(
    "--poem-top",
    `${Math.round(poemTop)}px`
  );

  // Mantiene la enfermera visible sobre las flores.
  document.documentElement.style.setProperty(
    "--nurse-bottom",
    `${Math.round(nurseBottom)}px`
  );
...
function createScene() {
  ...
  const tulipCount = Math.floor(isMobile ? 42 : 170);
  ...
  for (let i = 0; i < tulipCount; i += 1) {
    ...
    const y = height * random(isMobile ? 0.9 : 0.72, 1.04);
    const size = random(isMobile ? 6 : 10, isMobile ? 13 : 25) * (0.65 + depth * 0.9);

    tulips.push({
      x: random(-40, width + 40),
      y,
      size,
      height: random(isMobile ? 16 : 42, isMobile ? 38 : 100) * (0.72 + depth),
      colors: pick(palette),
      sway: random(0.8, 1.8),
...
function drawCloud(cloud, time) {
  ...
  const y =
    cloud.y +
    Math.sin(time * 0.0005 + cloud.scale) * 5 +
    pointer.y * cloud.scale * (isMobile ? 2 : 5);
...
function drawField() {
  const fieldTop = height * (isMobile ? 0.86 : 0.66);
  ...
  deliveryRows.forEach((rowData) => {
    ...
    rowData.words.forEach((wordText) => {
      ...
      [...wordText].forEach((char) => {
        ...
        span.className = char === "💜" ? "neon-letter heart-symbol" : "neon-letter";
